<img src="cover.jpeg" align="right" >

# Curso Java Completo - Nélio Alves



- [x] Módulo 01 — Introdução ao curso
- [x] Módulo 02 — Conceitos de programação
- [x] Módulo 03 — Introdução à linguagem Java
- [x] Módulo 04 — Estrutura sequencial
- [x] Módulo 05 — Estrutura condicional
- [ ] Módulo 06 — Estruturas repetitivas
- [ ] Módulo 07 — Outros tópicos básicos de Java
- [ ] Módulo 08 — Introdução à Programação Orientada a Objetos (POO)
- [ ] Módulo 09 — Construtores, palavra `this`, sobrecarga, encapsulamento
- [ ] Módulo 10 — Arrays e Listas
- [ ] Módulo 11 — Tópicos especiais em Java (Data/Hora)
- [ ] Módulo 12 — Enumerações e Composição
- [ ] Módulo 13 — Herança e Polimorfismo
- [ ] Módulo 14 — Tratamento de Exceções
- [ ] Módulo 15 — Trabalhando com Arquivos
- [ ] Módulo 16 — Interfaces
- [ ] Módulo 17 — Generics, Set e Map
- [ ] Módulo 18 — Programação Funcional + Lambda
- [ ] Módulo 19 — JDBC e Banco de Dados
- [ ] Módulo 20 — JavaFX (GUI)
- [ ] Módulo 21 — Git / GitHub
- [ ] Módulo 22 — Spring Boot + JPA + Hibernate
- [ ] Módulo 23 — API REST
- [ ] Módulo 24 — Projeto final

___
## **Projetos**

- [ ] Projeto: Sistema de Conta Bancária
- [ ] Projeto: Sistema de Pedido
- [ ] Projeto: Jogo de Xadrez
- [ ] Projeto: API REST com Spring Boot

____
